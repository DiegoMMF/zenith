...
class TestSubmitPlanValidation:
    def test_contract_less_plan_rejected(self, config: HarnessConfig, workspace: Path) -> None:
        controller = ProjectController(
            config,
            MockDispatcher(lambda req: WorkHandoff(node_id=req.task.id, done=True, report="ok")),
            MockTerminalReviewer(TerminalReviewHandoff(done=True, report="")),
        )
